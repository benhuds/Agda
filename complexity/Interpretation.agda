...
module Interpretation where
  ...
  ren-eq-l-lam-lem : ∀ {Γ Γ' τ1} → (x : CTp) (ρ : rctx Γ (x :: Γ')) (k : fst [ Γ ]c) (x₁ : fst [ τ1 ]t)
                   → Preorder-str.≤ (snd [ Γ' ]c) (Monotone.f (interpR (throw-r (throw-r (r-extend {_} {_} {τ1} ρ)))) (k , x₁)) (Monotone.f (interpR (throw-r ρ)) k)
  ren-eq-l-lam-lem {Γ' = []} x ρ k x₁ = <>
  ren-eq-l-lam-lem {Γ' = x :: Γ'} x₁ ρ k x₂ = ren-eq-l-lam-lem x (throw-r ρ) k x₂ , (Preorder-str.refl (snd [ x ]t) (Monotone.f (lookup (ρ (iS i0))) k))
  ...
  ren-eq-l-lam {Γ' = []} {τ = τ} ρ e k x = Preorder-str.refl (snd [ τ ]t) (Monotone.f (interpE e) (<> , x))
  ren-eq-l-lam {Γ' = x :: Γ'} {τ = τ} {τ1 = τ1} ρ e k x₁ =
    Monotone.is-monotone (interpE e)
      ((Monotone.f (interpR (λ x₂ → iS (ρ (iS x₂)))) (k , x₁) , Monotone.f (lookup (ρ i0)) k) , x₁)
      ((Monotone.f (interpR (λ x₂ → ρ (iS x₂))) k , Monotone.f (lookup (ρ i0)) k) , x₁)
        ((ren-eq-l-lam-lem x ρ k x₁ , Preorder-str.refl (snd [ x ]t) (Monotone.f (lookup (ρ i0)) k)) , Preorder-str.refl (snd [ τ1 ]t) x₁)

  ren-eq-l-rec : ∀ {Γ Γ' τ} → (ρ : rctx Γ Γ') (e₂ : nat :: τ :: Γ' |- τ) (k : fst [ Γ ]c) (n : Nat) (x₂ : fst [ τ ]t)
               → Preorder-str.≤ (snd [ τ ]t)
                   (Monotone.f (interpE e₂) (Monotone.f (interpR (r-extend {_} {_} {nat} (r-extend {_} {_} {τ} ρ))) ((k , x₂) , n)))
                   (Monotone.f (interpE e₂) ((Monotone.f (interpR ρ) k , x₂) , n))
  ren-eq-l-rec {Γ' = []} {τ = τ} ρ e₂ k n x₂ = Preorder-str.refl (snd [ τ ]t) (Monotone.f (interpE e₂) ((<> , x₂) , n))
  ren-eq-l-rec {Γ' = x :: Γ'} {τ = τ} ρ e₂ k n x₂ =
    Monotone.is-monotone (interpE e₂)
      ((((Monotone.f (interpR (λ x₁ → iS (iS (ρ (iS x₁))))) ((k , x₂) , n)) , (Monotone.f (lookup (ρ i0)) k)) , x₂) , n)
      ((((Monotone.f (interpR (λ x₁ → ρ (iS x₁))) k) , (Monotone.f (lookup (ρ i0)) k)) , x₂) , n)
        ((({!ren-eq-l-lam-lem x (iS) k x₂!} , Preorder-str.refl (snd [ x ]t) (Monotone.f (lookup (ρ i0)) k)) , Preorder-str.refl (snd [ τ ]t) x₂) , ♭nat-refl n)
  ...
  s-id-l {Γ} {τ} (rec e e₁ e₂) k =
    Preorder-str.trans (snd [ τ ]t)
      ...
      (natrec (Monotone.f (interpE (subst e₁ ids)) k) (λ n x₂ → Monotone.f (interpE (subst e₂ (s-extend (s-extend ids)))) ((k , x₂) , n))
        ...
        (rec-lem k (Monotone.f (interpE e₁) k) (Monotone.f (interpE (subst e₁ ids)) k)
                   (λ n x₂ → Monotone.f (interpE e₂) ((k , x₂) , n))
                   (λ n x₂ → Monotone.f (interpE (subst e₂ (s-extend {_} {_} {nat} (s-extend {_} {_} {τ} ids)))) ((k , x₂) , n))
                   (Monotone.f (interpE (subst e ids)) k)
...
{-Preorder-str.≤ (snd [ τ ]t)
  ...
  s-id-r 1C k = <>
  s-id-r (plusC e e₁) k =
    plus-lem (Monotone.f (interpE (subst e (λ x → var x))) k) (Monotone.f (interpE (subst e₁ (λ x → var x))) k)
...
{-Preorder-str.≤ (snd [ τ ]t)
  ...
  s-comp2-l-lem-lem {Γ' = x :: Γ'} x₁ Θ v1 v2 k =
    ...
    (Preorder-str.trans (snd [ x ]t)
      ...
      (Monotone.f (interpE (Θ (iS i0))) k)
        ...
        (Preorder-str.trans (snd [ x ]t)
          (Monotone.f (interpE (subst (ren (Θ (iS i0)) iS) (lem4 v1 v2 sr iS))) k)
          (Monotone.f (interpE (subst (Θ (iS i0)) ids)) k)
          (Monotone.f (interpE (Θ (iS i0))) k)
            {!!}
            (s-id-r (Θ (iS i0)) k)))
  ...
  s-comp-l-lem : ∀ {Γ Γ' τ τ1} (Θ : sctx Γ Γ') (v : Γ |- τ1) (e : τ1 :: Γ' |- τ) (k : fst [ Γ ]c)
               → Preorder-str.≤ (snd [ Γ' ]c)
                   (Monotone.f (interpS (λ x → subst (ren (Θ x) iS) (lem3' ids v))) k)
                   (Monotone.f (interpS (λ x → Θ x)) k)
  s-comp-l-lem {Γ' = []} Θ v e k = <>
  s-comp-l-lem {Γ' = x :: Γ'} Θ v e k = 
  ...
  s-comp2-l-lem : ∀ {Γ Γ' τ τ1 τ2} (Θ : sctx Γ Γ') (v1 : Γ |- τ1) (v2 : Γ |- τ2) (e1 : τ1 :: τ2 :: Γ' |- τ) (k : fst [ Γ ]c)
                → Preorder-str.≤ (snd [ Γ' ]c)
                    (Monotone.f (interpS (λ x → subst (ren (ren (Θ x) iS) iS) (lem3' (lem3' ids v2) v1))) k)
                    (Monotone.f (interpS (λ x → Θ x)) k)
  s-comp2-l-lem {Γ' = []} Θ v1 v2 e1 k = <>
  s-comp2-l-lem {Γ' = x :: Γ'} Θ v1 v2 e1 k =
    ...
    (Preorder-str.trans (snd [ x ]t)
      ...
      (Monotone.f (interpE (Θ i0)) k)
        ...
        (Preorder-str.trans (snd [ x ]t)
          (Monotone.f (interpE (subst (ren (Θ i0) iS) (lem4 v1 v2 sr iS))) k)
          {!!} --(Monotone.f (interpE (subst (Θ i0) ids)) k)
          (Monotone.f (interpE (Θ i0)) k)
            {!s-id-r (ren (Θ i0) iS) ?!}
            {!!})) --(s-id-r (Θ i0) k))) (s-id-r (Θ i0) k)))

  s-comp-r-lem : ∀ {Γ Γ' τ τ1} (Θ : sctx Γ Γ') (v : Γ |- τ1) (e : τ1 :: Γ' |- τ) (k : fst [ Γ ]c)
               → Preorder-str.≤ (snd [ Γ' ]c)
                   (Monotone.f (interpS (λ x → Θ x)) k)
                   (Monotone.f (interpS (λ x → subst (ren (Θ x) iS) (lem3' (λ x₁ → var x₁) v))) k)
  s-comp-r-lem {Γ' = []} Θ v e k = <>
  s-comp-r-lem {Γ' = x :: Γ'} Θ v e k =
...
{- Preorder-str.≤ (snd [ x ]t) (Monotone.f (interpE (Θ i0)) k)
      (Monotone.f (interpE (Θ' i0)) k)-}

  s-cong2-lem : ∀ {Γ Γ' τ} (Θ Θ' : sctx Γ Γ') (e : Γ' |- τ) (x : (τ₁ : CTp) (x₁ : τ₁ ∈ Γ') → Θ x₁ ≤s Θ' x₁) (k : fst [ Γ ]c)
              → Preorder-str.≤ (snd [ τ ]t) (Monotone.f (interpE (subst e Θ)) k) (Monotone.f (interpE (subst e Θ')) k)
  s-cong2-lem {τ = τ} Θ Θ' e x k =
    Preorder-str.trans (snd [ τ ]t)
      ...
      (Monotone.f (interpE (subst e Θ')) k)
        ...
        (Preorder-str.trans (snd [ τ ]t)
          ...
          (Monotone.f (interpE (subst e Θ')) k)
            (Monotone.is-monotone (interpE e)
              (Monotone.f (interpS Θ) k) (Monotone.f (interpS Θ') k)
              {!!})
            (subst-eq-r Θ' e k))
      ...
      (Monotone.f (interpE e) (k , Monotone.f (interpE e2) k))
        ...
        (Monotone.is-monotone (interpE e)
          ...
          (k , Monotone.f (interpE e2) k)
            ({!!} , (Preorder-str.refl (snd [ τ' ]t) (Monotone.f (interpE e2) k))))
              ...
              (Monotone.f (interpE (subst e (lem3' Θ v))) k)
                (Monotone.is-monotone (interpE e)
                  ...
                  (Monotone.f (interpS (lem3' Θ v)) k)
                    (s-comp-l-lem Θ v e k , (Preorder-str.refl (snd [ τ1 ]t) (Monotone.f (interpE v) k))))
             ...
             (Preorder-str.trans (snd [ τ ]t)
               ...
               (Monotone.f (interpE e) (Monotone.f (interpS (lem3' ids v ss s-extend Θ)) k))
                 ...
                 (Monotone.is-monotone (interpE e)
                   (Monotone.f (interpS (lem3' Θ v)) k)
                   (Monotone.f (interpS (lem3' ids v ss s-extend Θ)) k)
                     (s-comp-r-lem Θ v e k , (Preorder-str.refl (snd [ τ1 ]t) (Monotone.f (interpE v) k)))))
  ...
  sound {τ = τ} ._ .(subst e1 (lem3' (lem3' Θ v2) v1)) (subst-compose2-l {_} {_} {.τ} {τ1} {τ2} Θ e1 v1 v2) k =
    Preorder-str.trans (snd [ τ ]t)
      (Monotone.f (interpE (subst (subst e1 (s-extend (s-extend Θ))) (lem3' (lem3' ids v2) v1))) k)
