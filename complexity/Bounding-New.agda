...
open import Bounding-Lemmas-New

module Bounding-New where
  ...
  boundingRec : ∀ {τ} (v : [] Source.|- nat) (val-v : val v)
              ...
              → ((v' : [] Source.|- nat) (val-v' : val v') (E' :  [] Pilot.|- nat) → valBound v' val-v' E'
                → (r : [] Source.|- susp τ) (val-r : val r) (R :  [] Pilot.|- || τ ||) → valBound r val-r R
                → expBound (Source.subst e1 (Source.lem4 v' r)) (Pilot.subst E1 (Pilot.lem4 E' R)))
  ...
  bounding : ∀{Γ τ} → (e : Γ Source.|- τ) → (Θ : Source.sctx [] Γ) 
                       ...
                       → (Θ' : Pilot.sctx [] ⟨⟨ Γ ⟩⟩c) 
                       → substBound Θ a Θ' 
                       → expBound (Source.subst e Θ) (Pilot.subst || e ||e Θ')
  ...
  bounding (app e1 e2) Θ a Θ' sb v val-v .((n0 +c n1) +c n)
    ...
    weakeningVal' val-v (snd IH1a) r-proj-s
      where
        ...
        IH1a = snd IH1 v2 v2-val (r-proj (Pilot.subst || e2 ||e Θ')) (snd IH2) v val-v n subst-evals 
